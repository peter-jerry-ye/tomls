...
class ArrayTest extends AnyFunSuite with Matchers with Inside with PartialFunctionValues with ScalaCheckPropertyChecks {
  test("Parse arrays with same type") {
    inside(PValue.parser.parseAll("[ 1, 2, 3 ]")) { case Right(InlineArray(array)) =>
      array should be(List(TInteger(1), TInteger(2), TInteger(3)))
    }
    inside(PValue.parser.parseAll("[ \"red\", \"yellow\", \"green\" ]")) { case Right(InlineArray(array)) =>
      array should be(List(TString("red"), TString("yellow"), TString("green")))
    }

    inside(PValue.parser.parseAll("[ [ 1, 2 ], [3, 4, 5] ]")) { case Right(InlineArray(array)) =>
      array should be(
        List(InlineArray(List(TInteger(1), TInteger(2))), InlineArray(List(TInteger(3), TInteger(4), TInteger(5))))
      )

    }
    inside(PValue.parser.parseAll("[ [ 1, 2 ], [\"a\", \"b\", \"c\"] ]")) { case Right(InlineArray(array)) =>
      array should be(
        List(InlineArray(List(TInteger(1), TInteger(2))), InlineArray(List(TString("a"), TString("b"), TString("c"))))
      )

    }
    inside(PValue.parser.parseAll("[ \"all\", 'strings', \"\"\"are the same\"\"\", '''type''' ]")) {
      case Right(InlineArray(array)) =>
        array should be(List(TString("all"), TString("strings"), TString("are the same"), TString("type")))
    }
  }

  test("Parse arrays with mixed types") {
    inside(PValue.parser.parseAll("[ 0.1, 0.2, 0.5, 1, 2, 5 ]")) { case Right(InlineArray(array)) =>
      array should be(List(TFloat(0.1), TFloat(0.2), TFloat(0.5), TInteger(1), TInteger(2), TInteger(5)))
  ...
  test("Parse arrays with multiline") {
    inside(
      PValue.parser
        ...
        |  1, 2, 3
        |]""".stripMargin)
    ) { case Right(InlineArray(array)) => array should be(List(TInteger(1), TInteger(2), TInteger(3))) }
    inside(
      PValue.parser
        .parseAll("""[
        |  1,
        |  2, # 这是可以的
        |]""".stripMargin)
    ) { case Right(InlineArray(array)) => array should be(List(TInteger(1), TInteger(2))) }
  }
